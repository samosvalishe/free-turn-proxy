...
> Вместо длинного перечисления флагов (`-peer`, `-mode`, `-obf-key` и т.д.) вы можете передать клиенту одну компактную ссылку `freeturn://` первым аргументом, или использовать флаг `-sub` для скачивания настроек с удаленного сервера! Подробнее читайте в [uri.md](uri.md) и [sub.md](sub.md).
